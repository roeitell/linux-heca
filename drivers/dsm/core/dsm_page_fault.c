...
static int dsm_try_pull_req_complete(struct tx_buf_ele *tx_e) {
        ...
        if (unlikely(tx_e->dsm_msg->type == TRY_REQUEST_PAGE_FAIL)) {
                ...
                if (likely(r)) {
                        ...
                        unlock_page(dpc->pages[0]);
                        atomic64_inc(
                                        &dpc->svm->svm_sysfs.stats.nb_page_pull_fail);
...
static struct page *get_remote_dsm_page(struct vm_area_struct *vma,
                ...
                struct subvirtual_machine *remote_svm, unsigned long private,
                int tag, int i) {
...
static int get_dsm_page(struct mm_struct *mm, unsigned long addr,
                struct subvirtual_machine *fault_svm, unsigned long private,
                int tag);
...
static struct dsm_page_cache *dsm_cache_add_send(
        ...
        do {
                ...
                spin_lock_irq(&fault_svm->page_cache_spinlock);
                r = radix_tree_insert(&fault_svm->page_cache, norm_addr,
                                new_dpc);
                spin_unlock_irq(&fault_svm->page_cache_spinlock);
                radix_tree_preload_end();
                ...
                if (likely(!r)) {
                        ...
                        for (r = 0; r < svms.num; r++) {
                                get_remote_dsm_page(vma, norm_addr, new_dpc,
                                                fault_svm, svms.pp[r], private,
                                                tag, r);
                        }
                        if (prefetch) {
                                for (r = 1; r < 40; r++) {
                                        get_dsm_page(mm, addr + r * PAGE_SIZE,
                                                        fault_svm, 0,
                                                        PREFETCH_TAG);
                                }
                        }
                        ...
                        if (!pte_none(pte_entry)) {
                                ...
                                if (non_swap_entry(swp_e)) {
                                        if (is_dsm_entry(swp_e)) {
                                                dsd = swp_entry_to_dsm_data(
                                                                swp_e);
                                                dsm_cache_add_send(fault_svm,
                                                                dsd.svms, addr,
                                                                norm_addr, 2,
                                                                tag, vma, mm,
                                                        private , 0, pte_entry,
...
//we need to use the page addr and not the fault address in order to have a unique reference
        ...
        if (!dpc) {
                dpc = dsm_cache_add_send(fault_svm, dsd.svms, address,
                                norm_addr, 3, PULL_TAG, vma, mm, 0,
                                flags & FAULT_FLAG_ALLOW_RETRY, orig_pte,
                                page_table);
                if (unlikely(!dpc)) {
                        page_table = pte_offset_map_lock(mm, pmd, address,
                                        &ptl);
...
//we should pretty much always get in there unless we read fault
        ...
        if (flags & FAULT_FLAG_WRITE) {
                ret |= do_wp_dsm_page(fault_svm, mm, vma, address, page_table,
                                pmd, ptl, pte, norm_addr);
