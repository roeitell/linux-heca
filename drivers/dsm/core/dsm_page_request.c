                ...
                if (pte_none(pte_entry))
                        ...
                        if (non_swap_entry(swp_e)) {
                                if (is_dsm_entry(swp_e)) {
                                        dpc = dsm_cache_get_hold(local_svm,
                                                        addr);
                                        if (unlikely(!dpc))
...
// DSM3 : follow_page uses - goto bad_page; when !ZERO_PAGE..? wtf
        ...
        if (unlikely(PageKsm(page))) {
                ...
                r = ksm_madvise(pd.vma, addr, addr + PAGE_SIZE,
                                MADV_UNMERGEABLE, &(pd.vma->vm_flags));
...
static struct page *try_dsm_extract_page(struct subvirtual_machine *local_svm,
        ...
        noop: rcu_read_lock();
        atomic_dec(&dpc->nproc);
        r = (find_first_bit(&dpc->bitmap, dpc->svms.num) >= dpc->svms.num && atomic_cmpxchg(&dpc->nproc, 1, 0) == 1);
        rcu_read_unlock();
        if (r) {
...
struct page *dsm_extract_page_from_remote(struct dsm *dsm,
        ...
        if (tag == TRY_REQUEST_PAGE) {
                if (page)
                        ...
                        atomic64_inc(
                                        &local_svm->svm_sysfs.stats.nb_page_pull_fail);
        } else {
                if (page)
                        atomic64_inc(&local_svm->svm_sysfs.stats.nb_page_sent);
                else
...
struct page *dsm_prepare_page_for_push(struct subvirtual_machine *local_svm,
        ...
        if (unlikely(PageKsm(page))) {
                if (ksm_madvise(pd.vma, addr, addr + PAGE_SIZE,
                                MADV_UNMERGEABLE, &pd.vma->vm_flags))
                        goto bad_page;
        }
