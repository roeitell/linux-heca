...
int create_dsm(struct private_data *priv_data, __u32 dsm_id)
{
    ...
    /* FIXME: move this spin lock to be part of dsm_state */
    spin_lock(&dsm_lock);
...
int create_svm( __u32 dsm_id, __u32 svm_id, int local)
{
    ...
    if (!is_svm_local(new_svm)) {
        ...
        new_svm->descriptor = dsm_get_descriptor(dsm->dsm_id, svm_id);
    }
        ...
        struct conn_element *ele)
{
    ...
    for (i = 0; i < ele->tx_buffer.len; i++) {
        ...
        int types = REQUEST_PAGE | TRY_REQUEST_PAGE | PAGE_REQUEST_FAIL |
            REQUEST_READ;
        ...
        struct tx_buffer *tx)
{
    ...
    list_for_each_entry_safe (req, n, &tx->ordered_request_queue, ordered_list){
        if (req->src_id == svm_id || req->dest_id == svm_id) {
...
void remove_svm(u32 dsm_id, u32 svm_id)
{
     ...
     * refcounting. and yet we try to catch every operation, and be a surroagate
     ...
     * which it isn't!
     * FIXME: same problem is valid for push operations!
     */
    if (is_svm_local(svm)) {
        struct rb_root *root;
        struct rb_node *node;
        ...
        int unmap)
{
    ...
    mr->descriptor = dsm_get_descriptor(dsm->dsm_id, svm_ids);
    if (!mr->descriptor) {
