...
static int register_dsm(struct private_data *priv_data, void __user *argp) {
        ...
        do {
                ...
                if (found_dsm) {
                        printk(
                                        "[register_dsm] we already have the dsm in place \n");
                        break;
                }
...
static int register_svm(struct private_data *priv_data, void __user *argp) {
        ...
        do {
                ...
                if (likely(!r)) {
                        ...
                        printk(
                                        "[DSM_SVM] reg svm %p, res %d, dsm_id %u, svm_id: %u\n",
                                        new_svm, r, svm_info.dsm_id,
                                        svm_info.svm_id);
                        goto out;
...
static int connect_svm(struct private_data *priv_data, void __user *argp) {
        ...
        do {
                ...
                if (likely(!r)) {
                        ...
                        if (!cele) {
                                r = create_connection(dsm_state->rcm,
                                                &svm_info);
                                if (r)
                                        goto connect_fail;
...
//     Make sure specific MR not already created.
        ...
        for (j = 0; udata.svm_ids[j]; j++) {
                struct subvirtual_machine *svm = find_svm(dsm,
                                udata.svm_ids[j]);
                if (!svm)
                        goto out;
...
static int unmap_range(struct private_data *priv_data, void __user *argp) {
        ...
        for (j = 0; udata.svm_ids[j]; j++) {
                ...
                if (!svm) {
                        printk(
                                        "[UNMAP_RANGE] could not find the route element \n");
                        r = -1;
                        printk("[UNMAP_RANGE] dsm_id : %d - vm_id : %d\n",
                                        udata.dsm_id, udata.svm_ids[j]);
                        goto out;
                }
...
static int dsm_init(void) {
        ...
        reg_dsm_functions(&find_dsm, &find_svm, &find_local_svm_in_dsm,
                        &request_dsm_page, &dsm_request_page_pull);
