...
static int create_rx_buffer(struct conn_element *ele) {
        ...
        struct rx_buf_ele * rx = kmalloc(
                        (sizeof(struct rx_buf_ele) * RX_BUF_ELEMENTS_NUM),
                        GFP_KERNEL);
...
static int create_tx_buffer(struct conn_element *ele) {
        ...
        struct tx_buf_ele * tx_buff_e = kmalloc(
                        (sizeof(struct tx_buf_ele) * TX_BUF_ELEMENTS_NUM),
                        GFP_KERNEL);
...
void create_page_pull_request(struct conn_element *ele,
                struct tx_buf_ele * tx_e, u32 dsm_id, u32 local_id,
                ...
                if (likely(rcm->cm_id)) {
                        ...
                        if (rcm->cm_id->qp)
                                if ((r = ib_destroy_qp(rcm->cm_id->qp)))
                                        printk(
                                                        ">[destroy_rcm] - Cannot destroy qp %d\n",
                                                        r);

                        if (rcm->listen_cq)
                                if ((r = ib_destroy_cq(rcm->listen_cq)))
                                        printk(
                                                        ">[destroy_rcm] - Cannot destroy cq %d\n",
                                                        r);

                        if (rcm->mr)
                                if ((r = ib_dereg_mr(rcm->mr)))
                                        printk(
                                                        ">[destroy_rcm] - Cannot dereg mr %d\n",
                                                        r);

                        if (rcm->pd)
                                if ((r = ib_dealloc_pd(rcm->pd)))
                                        printk(
                                                        ">[destroy_rcm] - Cannot dealloc pd %d\n",
                                                        r);
        ...
        for (node = rb_first(root); node; node = rb_next(node)) {
                ...
                for (i = 0; i < svms.num; i++) {
                        if (svms.pp[i] && svms.pp[i]->svm_id == svm->svm_id) {
                                ...
                                svm_ids = svm_ids_without_svm(svms,
                                                svm->svm_id);
                                if (svm_ids[0]) {
...
void release_push_elements(struct subvirtual_machine *local_svm,
        ...
        for (node = rb_first(&local_svm->push_cache); node;
                        node = rb_next(node)) {
                rcu_read_lock();
...
void release_svm_tx_elements(struct subvirtual_machine *svm,
        ...
        for (i = 0; i < TX_BUF_ELEMENTS_NUM; i++) {
                ...
                if (msg->dsm_id == svm->dsm->dsm_id && (msg->src_id == svm->svm_id || msg->dest_id == svm->svm_id) && atomic_cmpxchg(&tx_buf[i].used, 1, 2) == 1) {
                        release_dpc_element(svm, tx_buf[i].wrk_req->dpc,
                                        &tx_buf[i]);
