        ...
        unsigned long addr, struct subvirtual_machine *remote_svm)
{
    ...
    if (likely(dpc) && remote_svm) {
        for_each_online_svm(dpc->svms, i) {
...
int dsm_pte_present(struct mm_struct *mm, unsigned long addr)
{
    ...
    /*
     * we don't have to be 100% accurate here, just make sure the pagetable
...
/* returns no. of pages unmapped (0 or 1), or -EFAULT on failure */
...
retry:
    ...
    *page = vm_normal_page(pd.vma, addr, *(pd.pte));
    ...
    pte_entry = ptep_clear_flush_notify(pd.vma, addr, pd.pte);
    if (read_copy) {
        pte_entry = pte_mkclean(pte_wrprotect(pte_entry));
        dsm_add_reader(local_svm, addr, remote_svm->svm_id);
    } else {
        pte_entry = dsm_descriptor_to_pte(remote_svm->descriptor, DSM_INFLIGHT);
        page_remove_rmap(*page);
        page_cache_release(*page);
        dec_mm_counter(mm, MM_ANONPAGES);
    ...
    set_pte_at_notify(mm, addr, pd.pte, pte_entry);
...
static int _push_back_if_remote_dsm_page(struct page *page)
{
    ...
    list_for_each_entry(avc, &anon_vma->head, same_anon_vma)
    {
        ...
        if (discarded < 0) {
            if (discarded == -EEXIST) {
                dsm_request_page_pull(svm->dsm, svm, page, address,
                        vma->vm_mm, mr);
            }
            if (unlikely(PageSwapCache(page)))
                try_to_free_swap(page);
...
/* gup from task context, mm already used by current */
...
retry:
    ...
    if (PageKsm(page)) {
        ...
        pte_unmap_unlock(pte, ptl);
        if (!ioctl_initiate_fault(mm, addr)) {
            printk("[dsm_flag_page_remote] ksm_madvise ret : %d\n", r);
