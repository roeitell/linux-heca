...
svm_sysfs = container_of(kobj, struct svm_sysfs , svm_kobject);
    ...
    if (strcmp(attr->attr.name, "local") == 0)
        ...
        var = 0;
    else if (strcmp(attr->attr.name, "nb_page_push") == 0)
        var = 0;
    else if (strcmp(attr->attr.name, "nb_page_push_fail") == 0)
        var = 0;
...
static struct kobj_attribute local_attribute =
        __ATTR(local, 0444, svm_show, NULL);
...
static struct kobj_attribute nb_page_push_attribute =
        __ATTR(nb_page_push, 0444, svm_show, NULL);
static struct kobj_attribute nb_page_push_fail_attribute =
        __ATTR(nb_page_push_fail, 0444, svm_show, NULL);
...
int create_svm_sysfs_entry(struct kobject* kobj, struct kobject *root_kobj,
        char* name) {
