...
static ssize_t svm_show(struct kobject *kobj, struct kobj_attribute *attr,
        ...
        if (strcmp(attr->attr.name, "nb_page_requested") == 0)
                ...
                var = atomic64_read(
                                &svm_sysfs->stats.nb_page_requested_prefetch);
...
int create_svm_sysfs_entry(struct subvirtual_machine *svm, char *ip) {
        ...
        r = kobject_init_and_add(kobj, &dsm_kobject_type,
                        &svm->dsm->dsm_kobject, id, ip);
        ...
        if (!r) {
                ...
                if (r) {
                        ...
                        r = kobject_init_and_add(kobj_ip, &dsm_kobject_type,
                                        kobj, ip);
                        if (r) {
                                kobject_put(kobj_ip);
                                kobject_del(kobj_ip);
...
int create_connection_sysfs_entry(struct con_element_sysfs *sysfs,
        ...
        int r = kobject_init_and_add(&sysfs->connection_kobject,
                        &dsm_kobject_type, root_kobj, name);

        if (!r) {
                r = kobject_init_and_add(&sysfs->connection_tx_kobject,
                                &dsm_kobject_type, &sysfs->connection_kobject,
                                "tx");
                if (!r) {
                        ...
                        if (!r) {
                                r = kobject_init_and_add(
                                                &sysfs->connection_rx_kobject,
                                                &dsm_kobject_type,
                                                &sysfs->connection_kobject,
                                                "rx");
                                if (!r)
